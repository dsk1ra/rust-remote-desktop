...
class FileTransferService {
  ...
  final StreamController<FileTransferState> _stateController = StreamController.broadcast();
  
  FileTransferSession? _currentSession;
  FileTransferState _currentState = FileTransferState(status: TransferStatus.idle);
  ...
  Future<void> sendOffer(File file) async {
    ...
    final id = DateTime.now().millisecondsSinceEpoch.toString(); // Simple ID for v1
    ...
    _updateState(FileTransferState(
  ...
  Future<void> _startTransferRobust() async {
     if (_currentSession == null || !_currentSession!.isSender) {
       print('FileTransfer: Cannot start transfer - session is null or not a sender');
       return;
     }
     
     print('FileTransfer: Starting robust transfer for ${_currentSession!.fileName}');
     ...
     try {
       ...
       while (offset < len) {
         ...
         // Read chunk
         final bytesToRead = (len - offset) > _chunkSize ? _chunkSize : (len - offset);
  ...
  void _handleFileChunk(List<int> chunk) {
    ...
    _updateState(_currentState.copyWith(
      bytesTransferred: _currentSession!.bytesReceived,
    ));
  }
  ...
  Future<void> _finalizeReceive() async {
    ...
    try {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        finalPath = '${downloadsDir?.path ?? (await getApplicationDocumentsDirectory()).path}/${_currentSession!.fileName}';
      }
