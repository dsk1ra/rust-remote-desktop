...
impl FileTransferService {
    pub async fn send_file(
        ...
        // Calculate global hash (in a real app, you might do this while sending or beforehand)
        // For simplicity and matching requirements, we'll do a quick pass or assume we have it.
        // The instructions say "Metadata Header: Send an initial JSON... containing global_hash".
    ...
    pub async fn receive_file(
        ...
        data_channel.on_message(Box::new(move |msg| {
            ...
            Box::pin(async move {
                if msg.is_string {
                    ...
                    // Binary chunk
                    // In v1 we assume raw bytes after metadata. 
                    // But our TransferMessage has Chunk variant.
                    // If we want raw bytes for performance, we might need a state machine.
        ...
        while let Some(msg) = rx.recv().await {
            ...
            // If we've received everything, break (since Eof might not be sent in v1 raw mode)
        ...
        if let (Some(m), Some(f)) = (metadata, file) {
            ...
            if final_hash == m.hash {
                ...
                drop(f); // Close file before renaming
